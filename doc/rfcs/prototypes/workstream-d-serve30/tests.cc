...
static void dump()
{
    ...
    std::printf("GOLD_3_0 %s\n", toHex(ser(V3_0, r)).c_str());
}
...
static bool roundtrips(Version v)
{
    ...
    if (v >= V3_0) eq = eq && got.logRef == r.logRef && got.failurePhase == r.failurePhase
...
int main(int argc, char ** argv)
{
    ...
    check(roundtrips(V3_0), "round-trip BuildResult @ 3.0");
    ...
    //    stops before the 3.0 tail; the leftover bytes are exactly that tail.
    {
        ...
        check(same28, "2.8 reader consumes exactly the 2.8 fields, no 3.0 fields");
        ...
        check(tail == coreOnly.buf, "leftover bytes are exactly the 3.0 diagnostic-core tail");
    }
    ...
    {
        Version neg = negotiate(V3_0, V2_8);            // = 2.8
        check(neg == V2_8, "min() handshake: 3.0 vs 2.8 -> 2.8");
        std::string b = ser(neg, r);
        check(b == ser(V2_8, r), "negotiated-down (2.8) emits no 3.0 tail");
        check(!supportsQueryBuildLog(neg), "QueryBuildLog not offered at negotiated 2.8");
    }

    // 6. Back-compat matrix (decisions B3 §3), both directions.
    check(negotiate(V2_8, V3_0) == V2_8, "old Hydra (2.8) <-> new Nix (3.0): negotiate 2.8");
    check(negotiate(V3_0, V2_8) == V2_8, "new Hydra (3.0) <-> old Nix (2.8): negotiate 2.8");
    check(negotiate(V3_0, V3_0) == V3_0 && supportsQueryBuildLog(negotiate(V3_0, V3_0)),
          "new <-> new: negotiate 3.0, full diagnostic core active");
    ...
    {
        std::string b30 = ser(V3_0, r), bU = ser(V3_unstable, r);
        check(bU.size() > b30.size() && bU.compare(0, b30.size(), b30) == 0,
              "unstable builderId/deduplicated append AFTER the frozen 3.0 core");
    }
