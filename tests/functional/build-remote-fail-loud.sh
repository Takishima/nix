...
# End-to-end test for the "fail loud" remote-build render (RFC
# `remote-build-protocol-redesign`, Gap C / G8): when a build fails on a
# *remote* builder, `build-remote.cc`'s `renderRemoteBuildLogTail` appends the
...
# failure was terse and the log was unreachable (Gap A).
#
# This drives the `ssh-ng://` path (a `RemoteStore`, which is now a `LogStore`
# fetching the log via the `QueryBuildLog` worker op — Phase 0). It uses
...
# And the same log must actually be fetchable that way (Gap A end-to-end).
