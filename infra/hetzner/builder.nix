...
let
  ...
  repo = "Takishima/nix";
  deployBranch = "claude/hetzner-nix-builder-setup-07r5ev";
...
in
{
  ...
  # Operations access for the bootstrap/CI key used by the bootstrap-builder
  # and builder-check workflows. Without it the installed system accepts no
  # SSH logins at all.
  ...
  services.comin = {
    enable = true;
    # Build the flake in this subdirectory, not the repo root (which is the Nix
    # package-manager flake).
    repositorySubdir = "infra/hetzner";
    remotes = [
      {
        name = "origin";
