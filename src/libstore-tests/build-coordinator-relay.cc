...
/* The ATTACHED handshake ack (and any future unknown tag) is decoder
   noise: the pump must skip it and decode the surrounding frames and the
   result exactly as without it. This is what keeps the handshake
   compatible in both directions — an old relay reading a new coordinator
   simply ignores the ack. */
TEST(CoordinatorRelay, attachedAckIsIgnoredByThePump)
{
    struct CapturingLogger : Logger
    {
        std::vector<std::string> buildLogLines;
    ...
    CapturingLogger capture;
    auto res = pump(
        {std::string(1, coordinator_proto::MSG_ATTACHED), frameRec("a line\n"), resultRec(successResult())}, capture);
