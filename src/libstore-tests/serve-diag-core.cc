...
static_assert(
    ...
    "until D3.1/D3.2/D3.4 are met (see remote-build-protocol-redesign.validation.md)");

namespace {

// ---- candidate serve 2.9 model (mirrors the prototype's serve.hh) -----------
...
// Golden bytes generated by the prototype (`make dump`); the characterisation
// guard against accidental layout drift while the layout soaks.
...
// ---- the tests (mirror the prototype's tests.cc) ----------------------------
...
// 6. Back-compat matrix (decisions B3 §3), both directions — at the COMPATIBLE
