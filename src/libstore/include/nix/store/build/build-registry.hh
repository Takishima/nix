...
///@file
///
/// The **Build Registry** — the server-side component that coalesces concurrent
 ...
 *
 * This is the load-bearing semantic commitment of the design. It is **never**
 * a store output path or the input-addressed `.drv`
 ...
 * resolved derivation it received and passes it in.
 */
 ...
 * control plane). The registry treats this as opaque identity and
 * defers the *authorization* decision to a pluggable `BuildAuthPolicy`, so no
 * call site inlines a `SO_PEERCRED` / same-host assumption.
 */
struct BuildAuth
{
...
/**
 * Pluggable authorization policy. The authorization check is the *same code*
 * whether the registry is hosted by the
...
/**
 * Per-subscriber options. Deadlines are **per-subscriber**: the
...
/**
 * Replay-buffer caps: a byte cap, default 4 MiB total, kept as a ~1 MiB
