...
typedef enum {
    ...
    resFetchStatus = 108,
    /**
     * The outcome of a build activity (one integer field: 0 = success).
     * Emitted while the activity is still live, so a logger buffering the
     * build's log can flush it; loggers that don't care ignore it.
     */
    resBuildResult = 109,
} ResultType;
