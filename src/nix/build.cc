...
/* The canonical `BuildResult` JSON plus the path it is keyed on, in
   this command's serialization style (store paths include the store
   dir, like the success entries above). */
...
#include "build.md"
    ...
    void run(ref<Store> store, Installables && installables) override
    {
        ...
        /* Fail exactly as `Installable::build` would have, but only after
           the JSON above made the failures machine-readable. */
