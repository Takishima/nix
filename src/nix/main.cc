...
void mainWrapped(int argc, char ** argv)
{
    ...
    /* Apply the `print-build-logs` setting to the logger. Only when it was
       explicitly set (via `-L`, `--option`, or `nix.conf`), so we don't
       clobber a mode chosen by `--log-format` (e.g. `bar-with-logs`). */
